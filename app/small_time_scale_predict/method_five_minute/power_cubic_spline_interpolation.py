...
def cubic_spline_interpolation_for_train(history_power=None, fifteenminutes_result=None):
    """
    三次条样插值
...
def cubic_spline_interpolation_for_predict(history_power=None, fifteenminutes_result=None, forecast_time=0):
    """
    三次条样插值
    :param history_power: 历史功率
    :param fifteenminutes_result: 15分钟分辨率的预测功率，共16个功率值
...
def predict_with_power_cubic_spline_interpolation_model(five_minute_model, feature, history_power, forecast_time):
    """
    使用三次条样插值法的模型（先预测15分钟分辨率功率，再使用三次条样插值法插值）开展预测
    ...
    # 插值
    predict_result_raw = cubic_spline_interpolation_for_predict(
...
def test_PCSI(config, test_feature, test_target, fifteen_model):
    """
    使用三次条样插值法的模型（先预测15分钟分辨率功率，再使用三次条样插值法插值）开展模型测试
    ...
    while end_index <= max_index:
        ...
        predict_result = cubic_spline_interpolation_for_train(history_power=history_power,
                                                              fifteenminutes_result=predict_result)
    ...
    five_minute_accuracy = result_evaluation_five_minute(predict_result_group, test_target_data,
                                                         config['online_capacity'],
                                                         predict_term="five_minute_ultra_short")
    # 结果统计分析
    # generate_page_five_minute(predict_result_group, test_target_data, station_name, five_minute_accuracy, method="PCSI",
    #                           save_page=True)
