...
class BLSTM(BaseModel):
    ...
    def __init__(self, input_size, params={'blstm_alpha': 0.35, 'blstm_units': 50, 'epochs': 30}, predict_type="wind",
                 irradiance=None):
        """
        初始化。待完善
        """
        # 初始化父类
        super(BLSTM, self).__init__(predict_type=predict_type, irradiance=irradiance)
        ...
        blstm_alpha = params['blstm_alpha']
        blstm_units = params['blstm_units']
        ...
        # BLSTM模型
        self.__model_blstm = keras.Sequential()
        self.__model_blstm.add(Bidirectional(keras.layers.LSTM(
              units=blstm_units,  # 输出维度
              batch_input_shape=(batch_size, time_steps, input_size),  # 输入维度
              stateful=False,  # 保持状态
           ), merge_mode='concat'))
        self.__model_blstm.add(keras.layers.LeakyReLU(alpha=blstm_alpha))
        self.__model_blstm.add(keras.layers.Dense(256))
        self.__model_blstm.add(keras.layers.LeakyReLU(alpha=blstm_alpha))
        self.__model_blstm.add(keras.layers.Dense(128))
        self.__model_blstm.add(keras.layers.LeakyReLU(alpha=blstm_alpha))
        self.__model_blstm.add(keras.layers.Dense(64))
        self.__model_blstm.add(keras.layers.LeakyReLU(alpha=blstm_alpha))
        self.__model_blstm.add(keras.layers.Dense(32))
        self.__model_blstm.add(keras.layers.LeakyReLU(alpha=blstm_alpha))
        self.__model_blstm.add(keras.layers.Dense(16))
        self.__model_blstm.add(keras.layers.LeakyReLU(alpha=blstm_alpha))
        self.__model_blstm.add(keras.layers.Dropout(0.5))
        # 输出层
        self.__model_blstm.add(keras.layers.Dense(1))
        self.__model_blstm.add(keras.layers.LeakyReLU(alpha=0.3))
        # 定义优化器
        self.__model_blstm.compile(loss='mse', metrics=['accuracy'])

        # BLSTM模型
        # 不使用历史功率数据的模型
        self.__model_blstm_withoutpower = keras.Sequential()
        self.__model_blstm_withoutpower.add(Bidirectional(keras.layers.LSTM(
            units=blstm_units,  # 输出维度
            batch_input_shape=(batch_size, time_steps, input_size - 1),  # 输入维度
            stateful=False,  # 保持状态
        ), merge_mode='concat'))
        self.__model_blstm_withoutpower.add(keras.layers.LeakyReLU(alpha=blstm_alpha))
        self.__model_blstm_withoutpower.add(keras.layers.Dense(256))
        self.__model_blstm_withoutpower.add(keras.layers.LeakyReLU(alpha=blstm_alpha))
        self.__model_blstm_withoutpower.add(keras.layers.Dense(128))
        self.__model_blstm_withoutpower.add(keras.layers.LeakyReLU(alpha=blstm_alpha))
        self.__model_blstm_withoutpower.add(keras.layers.Dense(64))
        self.__model_blstm_withoutpower.add(keras.layers.LeakyReLU(alpha=blstm_alpha))
        self.__model_blstm_withoutpower.add(keras.layers.Dense(32))
        self.__model_blstm_withoutpower.add(keras.layers.LeakyReLU(alpha=blstm_alpha))
        self.__model_blstm_withoutpower.add(keras.layers.Dense(16))
        self.__model_blstm_withoutpower.add(keras.layers.LeakyReLU(alpha=blstm_alpha))
        self.__model_blstm_withoutpower.add(keras.layers.Dropout(0.5))
        # 输出层
        self.__model_blstm_withoutpower.add(keras.layers.Dense(1))
        self.__model_blstm_withoutpower.add(keras.layers.LeakyReLU(alpha=0.3))
        # 定义优化器
        self.__model_blstm_withoutpower.compile(loss='mse', metrics=['accuracy'])

    def build_blstmmodel(self, input_size=6, blstm_alpha=0.3,
                         batch_size=10, time_steps=96, blstm_units=10):
        model = keras.Sequential()
        model.add(Bidirectional(keras.layers.LSTM(
            units=blstm_units,  # 输出维度
            batch_input_shape=(batch_size, time_steps, input_size),  # 输入维度
            stateful=False,  # 保持状态
        ), merge_mode='concat'))
    ...
    def save_model(self, filename):
        ...
        """
        model = self.__model_blstm
        modelwithout_power = self.__model_blstm_withoutpower
        # import tensorflow
        # tensorflow.saved_model.save(self.__model_grua, dir_path)
        ...
        self.__model_blstm = None
        self.__model_blstm_withoutpower = None
        joblib.dump(self, filename=filename)
        # 恢复原状
        self.__model_blstm = model
        self.__model_blstm_withoutpower = modelwithout_power

    def load_model(self, dir_path):
        """
        加载模型。
        """
        self.__model_blstm_withoutpower = tensorflow.keras.models.load_model(os.path.join(dir_path, "without_power"))
        self.__model_blstm = tensorflow.keras.models.load_model(
    ...
    def fit(self, feature_data, target_data, *args, **kwargs):
        ...
        # 模型适配
        self.__model_blstm_withoutpower.fit(X_fit, Y_fitdata, epochs=self.epochs, batch_size=96)
        ...
        Y_fitdata = Y_fit.flatten()
        # 模型适配
        self.__model_blstm.fit(X_fit, Y_fitdata, epochs=self.epochs, batch_size=96)
    ...
    def predict_without_history_power(self, feature, *args, **kwargs):
        ...
        :return:
        """
        result = (self.__model_blstm_withoutpower.predict(
    ...
    def predict_with_history_power(self, feature, history_power, *args, **kwargs):
        ...
        input_x = (numpy.c_[feature, history_power * numpy.ones(len(feature))]).reshape(len(feature), 1,
                                                                                        feature.shape[1] + 1)
        result = (self.__model_blstm.predict(input_x)).flatten()
        return result
