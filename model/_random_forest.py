...
class RandomForest(BaseModel):
    ...
    def __init__(self,
                 params={'bootstrap': True, 'max_features': 'sqrt', 'max_depth': 10, 'min_samples_leaf': 4, 'min_samples_split': 10, 'n_estimators': 100},
                 predict_type="wind", irradiance=None,
                 without_history_power=None):
        """
        初始化。待完善
        """
        # 初始化父类
        super(RandomForest, self).__init__(predict_type=predict_type, irradiance=irradiance,
        ...
        # RandomForest模型
        self.__model_rf = RandomForestRegressor(bootstrap=bootstrap, max_depth=max_depth,
                                                max_features=max_features, min_samples_leaf=min_samples_leaf,
                                                min_samples_split=min_samples_split, n_estimators=n_estimators,
                                                n_jobs=-1)
        self.__model_rf_withhp = RandomForestRegressor(bootstrap=bootstrap, max_depth=max_depth,
                                                       max_features=max_features, min_samples_leaf=min_samples_leaf,
                                                       min_samples_split=min_samples_split, n_estimators=n_estimators,
                                                       n_jobs=-1)
    ...
    def fit(self, feature_data, target_data, *args, **kwargs):
        ...
        if "without_history_power" not in kwargs or kwargs["without_history_power"] is None or kwargs["without_history_power"] is True:
            self.__model_rf.fit(feature_data, target_data)
        if "predict_term" not in kwargs or kwargs["predict_term"] is None or kwargs["predict_term"] == 'ultra_short':
            self.__model_rf_withhp.fit(numpy.hstack((feature_data[1:, :], target_data[:-1].reshape(-1, 1))),
                                       target_data[1:])
    ...
    def predict_without_history_power(self, feature, *args, **kwargs):
        ...
        :return:
        """
        result = self.__model_rf.predict(feature)
    ...
    def predict_with_history_power(self, feature, history_power, *args, **kwargs):
        ...
        for i in range(len(feature)):
            result[i] = self.__model_rf_withhp.predict(numpy.hstack((feature[i, :], history_power)).reshape(1, -1))
