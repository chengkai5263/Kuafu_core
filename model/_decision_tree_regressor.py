...
class DecisionTreeRegressor(BaseModel):
    """
    DecisionTreeRegressor模型，预测发电功率
    """
    def __init__(self, predict_type="wind", params={'n_estimators': 500}, irradiance=None, without_history_power=None):
        """
        初始化。待完善
        """
        # 初始化父类
        super(DecisionTreeRegressor, self).__init__(predict_type=predict_type, irradiance=irradiance,
        ...
        self.__model_DTR_without_history_power = BaggingRegressor(**params)
        self.__model_DTR_with_history_power = BaggingRegressor(**params)
    ...
    def fit(self, feature_data, target_data, *args, **kwargs):
        ...
        :return:
        """
        if kwargs["without_history_power"] is None or kwargs["without_history_power"] is True:
            self.__model_DTR_without_history_power.fit(feature_data, target_data)
        if kwargs["predict_term"] is None or kwargs["predict_term"] == 'ultra_short':
            self.__model_DTR_with_history_power.fit(numpy.hstack(
    ...
    def predict_without_history_power(self, feature, *args, **kwargs):
        ...
        :return:
        """
        result = self.__model_DTR_without_history_power.predict(feature)
    ...
    def predict_with_history_power(self, feature, history_power, *args, **kwargs):
        ...
        for i in range(len(feature)):
            result[i] = self.__model_DTR_with_history_power.predict(
