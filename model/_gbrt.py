...
def optimizer(model, train_data, target_data):
    # 设置想要优化的超参数以及他们的取值分布
    param_dist = {
        # 'n_estimators': numpy.linspace(100, 1000, num=10, endpoint=True, dtype=int)
        'n_estimators': range(100, 1000, 50),  # 要执行的推进阶段的数量
        'max_depth': range(2, 15, 1),  # 单个回归估计量的最大深度
        'learning_rate': numpy.linspace(0.01, 2, 20),  # 学习率
        'min_samples_split': range(5, 15, 1)  # 分割一个内部节点所需的最小样本数
    }
    # 开启超参数空间的随机搜索
    n_iter_search = 10
    random_search = RandomizedSearchCV(model, param_distributions=param_dist, n_iter=n_iter_search)
    random_search.fit(train_data, target_data)

    return random_search.best_estimator_


class GBRT(BaseModel):
    """
    GBRT 模型，预测发电功率
    """
    def __init__(self, predict_type="wind", params={'n_estimators': 500, 'max_depth': 4, 'min_samples_split': 5,
                                                    'learning_rate': 0.01, 'loss': 'ls'}, irradiance=None,
                 without_history_power=None):
        """
        初始化。待完善
        """
        # 初始化父类
        super(GBRT, self).__init__(predict_type=predict_type, irradiance=irradiance,
        ...
        # GBRT模型
        self.__model_GBRT_without_history_power = ensemble.GradientBoostingRegressor(**params)
        self.__model_GBRT_with_history_power = ensemble.GradientBoostingRegressor(**params)
    ...
    def fit(self, feature_data, target_data, *args, **kwargs):
        ...
        if "without_history_power" not in kwargs or kwargs["without_history_power"] is None or kwargs["without_history_power"] is True:
            self.__model_GBRT_without_history_power.fit(feature_data, target_data)

        if "predict_term" not in kwargs or kwargs["predict_term"] is None or kwargs["predict_term"] == 'ultra_short':
            self.__model_GBRT_with_history_power.fit(numpy.hstack(
                (feature_data[1:, :], target_data[:-1].reshape(1, -1).T)), target_data[1:].reshape(1, -1).T)
    ...
    def predict_without_history_power(self, feature, *args, **kwargs):
        ...
        :return:
        """
        result = self.__model_GBRT_without_history_power.predict(feature)
    ...
    def predict_with_history_power(self, feature, history_power, *args, **kwargs):
        ...
        for i in range(len(feature)):
            result[i] = self.__model_GBRT_with_history_power.predict(
                numpy.hstack((feature[i, :], history_power)).reshape(1, -1))
