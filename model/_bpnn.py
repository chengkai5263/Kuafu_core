...
class BPNN(BaseModel):
    ...
    def __init__(self, params={'hidden_layer_sizes': 100, 'learning_rate_init': 0.01}, max_iter=300, random_state=21,
                 predict_type="wind", irradiance=None, without_history_power=None):
        """
        初始化。待完善
        ...
        """
        # 初始化父类
        super(BPNN, self).__init__(predict_type=predict_type, irradiance=irradiance,
        ...
        # BP模型
        self.__model_bpnn_withoutp = MLPRegressor(hidden_layer_sizes=(hidden_layer_sizes,), activation="relu",
                                                  learning_rate_init=learning_rate_init, max_iter=max_iter,
                                                  random_state=random_state)
        self.__model_bpnn_withp = MLPRegressor(hidden_layer_sizes=(hidden_layer_sizes,), activation="relu",
                                               learning_rate_init=learning_rate_init, max_iter=max_iter,
                                               random_state=random_state)
    ...
    def fit(self, feature_data, target_data, *args, **kwargs):
        ...
        if "without_history_power" not in kwargs or kwargs["without_history_power"] is None or kwargs["without_history_power"] is True:
            self.__model_bpnn_withoutp.fit(feature_data, target_data)
        if "predict_term" not in kwargs or kwargs["predict_term"] is None or kwargs["predict_term"] == 'ultra_short':
            target_data = target_data * 5
            self.__model_bpnn_withp.fit(numpy.hstack((feature_data[1:, :], target_data[:-1].reshape(-1, 1))),
                                        target_data[1:])
    ...
    def predict_without_history_power(self, feature, *args, **kwargs):
        ...
        :return:
        """
        result = self.__model_bpnn_withoutp.predict(feature)
    ...
    def predict_with_history_power(self, feature, history_power, *args, **kwargs):
        ...
        :return:
        """
        history_power = history_power * 5
        result = numpy.zeros((len(feature)))
        for i in range(len(feature)):
            result[i] = self.__model_bpnn_withp.predict(numpy.hstack((feature[i, :], history_power)).reshape(1, -1))
