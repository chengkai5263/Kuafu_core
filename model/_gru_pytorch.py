...
class GRUPytorch(BaseModel):
    """
    GUR模型，预测发电功率
    """

    def __init__(self, input_size=1, params={'hidden_dim': 5, 'hidden_layer': 2}, predict_type="wind", irradiance=None):
        """
        初始化。待完善
        """
        # 初始化父类
        super(GRUPytorch, self).__init__(predict_type=predict_type, irradiance=irradiance)

        hidden_dim = params['hidden_dim']
        hidden_layer = params['hidden_layer']
        self.__model_gru = gruModel(in_dim=input_size, hidden_dim=hidden_dim, hidden_layer=hidden_layer)
    ...
    def fit(self, feature_data, target_data, *args, **kwargs):
        ...
        optimizer = optim.Adam(self.__model_gru.parameters(), lr=1e-2)
        # 训练模型
        for epoch in range(1500):
            # forward
            inputs = aaa.to(torch.float32)
            target = bbb.to(torch.float32)
            output = self.__model_gru(inputs)
    ...
    def predict_without_history_power(self, feature, *args, **kwargs):
        ...
        result = array(self.__model_gru(Variable(input_x)).data).reshape(-1)
...
class gruModel(nn.Module):
    def __init__(self, in_dim, hidden_dim, hidden_layer):
        super(gruModel, self).__init__()
        self.gruLayer = nn.GRU(in_dim, hidden_dim, hidden_layer)
        self.fcLayer = nn.Linear(hidden_dim, 1)

    def forward(self, x):
        input = x.transpose(0, 1)
        out, _ = self.gruLayer(input)
