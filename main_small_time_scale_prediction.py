...
from task.mysql.small_time_scale import small_time_scale
...
from task.mysql.cloud_trigger import start_feature_select, start_best_parameter_search, start_ensemble_learning, \
...
def test_serial_tasks_feature_parameter_ensemble(host, user, password, database, charset, port, rate=0.75,
    ...
    try:
        ...
        for i in range(result):
            ...
            try:
                c.execute("select * from nwp_" + str(station_id) + " limit 1;")
                db.commit()
                c.execute("select * from real_power_" + str(station_id) + " limit 1;")
                db.commit()
            ...
            try:
                ...
                try:
                    c.execute( "CREATE TABLE IF NOT EXISTS `predict_power_" + str(station_id) + "_train` ("
                    ...
                    logs.error(str(err), exc_info=True)
                    db.rollback()
                    logs.warning("%s%s%s%s" % (str(station_id), "表predict_power_", str(station_id), "_train创建失败，该场站无法参与测试！！！"))
                    c.execute("update configure set station_status = 0 where id  = %s;", station_id)
                    db.commit()
                    continue
...
class AppDaemon(Daemon):
    ...
    def set_normal_schedule_job(self, **kwargs):
        ...
        small_time_scale_ensemble_learning_database_conf = kwargs.get("database", {})
        trigger_of_small_time_scale_ensemble_learning_conf = kwargs.get("job_trigger_of_small_time_scale_ensemble_learning", {})
        if trigger_of_small_time_scale_ensemble_learning_conf.get("enable", True):
            self.scheduler.add_job(small_time_scale.add_ensemble_learning_task, 'interval', seconds=2, executor="default",
                                   kwargs={'host': small_time_scale_ensemble_learning_database_conf.get('host', None),
                                           'user': small_time_scale_ensemble_learning_database_conf.get('user', None),
                                           'password': small_time_scale_ensemble_learning_database_conf.get('password', None),
                                           'database': small_time_scale_ensemble_learning_database_conf.get('database', None),
                                           'charset': small_time_scale_ensemble_learning_database_conf.get('charset', None),
                                           'port': small_time_scale_ensemble_learning_database_conf.get('port', None)}
                                   )
            self.scheduler.add_job(small_time_scale.start_ensemble_learning, 'interval', seconds=2, executor="default",
                                   kwargs={'scheduler': self.scheduler, 'executor_name': "processpool",
                                           'host': small_time_scale_ensemble_learning_database_conf.get('host', None),
                                           'user': small_time_scale_ensemble_learning_database_conf.get('user', None),
                                           'password': small_time_scale_ensemble_learning_database_conf.get('password', None),
                                           'database': small_time_scale_ensemble_learning_database_conf.get('database', None),
                                           'charset': small_time_scale_ensemble_learning_database_conf.get('charset', None),
        ...
        small_time_scale_predict_database_conf = kwargs.get("database", {})
        trigger_of_small_time_scale_predict_conf = kwargs.get("job_trigger_of_small_time_scale_predict", {})
        if trigger_of_small_time_scale_predict_conf.get("enable", True):
            #
            self.scheduler.add_job(small_time_scale.small_time_scale_predict, 'date',
                                   executor="default",
                                   kwargs={'scheduler': self.scheduler, 'executor_name': "processpool",
                                           'host': small_time_scale_predict_database_conf.get('host', None),
                                           'user': small_time_scale_predict_database_conf.get('user', None),
                                           'password': small_time_scale_predict_database_conf.get('password', None),
                                           'database': small_time_scale_predict_database_conf.get('database', None),
                                           'charset': small_time_scale_predict_database_conf.get('charset', None),
                                           'port': small_time_scale_predict_database_conf.get('port', None)}
                                   )

        small_time_scale_transfer_learning_database_conf = kwargs.get("database", {})
        trigger_of_small_time_scale_transfer_learning_conf = kwargs.get("job_trigger_of_small_time_scale_transfer_learning", {})
        if trigger_of_small_time_scale_transfer_learning_conf.get("enable", True):
            self.scheduler.add_job(small_time_scale.add_transfer_learning_task, 'interval', seconds=2, executor="default",
                                   kwargs={'host': small_time_scale_transfer_learning_database_conf.get('host', None),
                                           'user': small_time_scale_transfer_learning_database_conf.get('user', None),
                                           'password': small_time_scale_transfer_learning_database_conf.get('password', None),
                                           'database': small_time_scale_transfer_learning_database_conf.get('database', None),
                                           'charset': small_time_scale_transfer_learning_database_conf.get('charset', None),
                                           'port': small_time_scale_transfer_learning_database_conf.get('port', None)}
                                   )
            self.scheduler.add_job(small_time_scale.start_transfer_learning, 'interval', seconds=2, executor="default",
                                   kwargs={
                                           'host': small_time_scale_transfer_learning_database_conf.get('host', None),
                                           'user': small_time_scale_transfer_learning_database_conf.get('user', None),
                                           'password': small_time_scale_ensemble_learning_database_conf.get('password', None),
                                           'database': small_time_scale_ensemble_learning_database_conf.get('database', None),
                                           'charset': small_time_scale_ensemble_learning_database_conf.get('charset', None),
