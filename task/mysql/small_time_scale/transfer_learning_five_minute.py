...
def transfer_learning(host, user, password, database, charset, port, station_id=None):
    ...
    if result == 1:
        ...
        try:
            weight_temp = transfer_config['weight']
        ...
        if farm_database.__contains__('station_status'):
            for val in similar_farm_list:
                ...
                try:
                    copy_file(transfer_config['model_savepath'][0] + val + "/five_minute",
                              transfer_config['model_savepath'][0] + str(target_farm['id'][0]) + "/five_minute")
                except Exception as err:
                    logs.error(str(err), exc_info=True)
                    logs.error("模型库中无" + val + "小时间尺度预测模型")
            ...
            try:
                ...
                # 将源场站的5分钟级最优模型写到数据库
                c.execute("select best_model_five_minute from syy_5_minute_ensemble_learning"
                          " where station_id=%s;", (eval(similar_farm_list[0])))
                db.commit()
                record = c.fetchall()
                best_model_five_minute = record[0][0]

                c.execute("update syy_transfer_study_5_minute set five_minute_best_model = %s"
                          " where station_id = %s;",
                          (best_model_five_minute, target_farm['id'][0]))
                db.commit()
